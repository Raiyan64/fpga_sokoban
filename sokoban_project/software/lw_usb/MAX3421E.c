...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	// create command line and write data
	tx_buf[0] = reg+0x02;
	tx_buf[1] = val;

	// transfer data
	Status = XSpi_Transfer(&SpiInstance, tx_buf, rx_buf, byteCount);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	if (data == NULL || nbytes == 0) return NULL;
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// create command line and dummy data
	tx_buf[0] = reg;
	for (int i = 0; i<nbytes; i++) {
		tx_buf[i+1] = 0x00;
	}
